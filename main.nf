...
//    genome_index_ch = Channel.fromPath(params.genome_index_files).collect().ifEmpty {
  ...
  //  indexed_genome_ch = indexGenome(genome_ch, genome_index_ch)
    ...
    // SCADD - put downsampled bams and original bam into same channel
    // https://www.nextflow.io/docs/latest/operator.html#mix
    //will need to change output file names in some of the processes if doing this
  //  all_bams_ch = downsample_ch.mix(align_ch)
